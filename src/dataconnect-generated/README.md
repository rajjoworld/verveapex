...
## GetNotesByNotebook
You can execute the `GetNotesByNotebook` query using the following action shortcut function, or by calling `executeQuery()` after calling the following `QueryRef` function, both of which are defined in [dataconnect-generated/index.d.ts](./index.d.ts):
```typescript
getNotesByNotebook(vars: GetNotesByNotebookVariables): QueryPromise<GetNotesByNotebookData, GetNotesByNotebookVariables>;
...
```typescript
getNotesByNotebook(dc: DataConnect, vars: GetNotesByNotebookVariables): QueryPromise<GetNotesByNotebookData, GetNotesByNotebookVariables>;
...
If you need the name of the operation without creating a ref, you can retrieve the operation name by calling the `operationName` property on the getNotesByNotebookRef:
```typescript
const name = getNotesByNotebookRef.operationName;
console.log(name);
```

### Variables
The `GetNotesByNotebook` query requires an argument of type `GetNotesByNotebookVariables`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
...
```
### Return Type
Recall that executing the `GetNotesByNotebook` query returns a `QueryPromise` that resolves to an object with a `data` property.

The `data` property is an object of type `GetNotesByNotebookData`, which is defined in [dataconnect-generated/index.d.ts](./index.d.ts). It has the following fields:
```typescript
...
```
### Using `GetNotesByNotebook`'s action shortcut function

```typescript
import { getDataConnect } from 'firebase/data-connect';
import { connectorConfig, getNotesByNotebook, GetNotesByNotebookVariables } from '@dataconnect/generated';

// The `GetNotesByNotebook` query requires an argument of type `GetNotesByNotebookVariables`:
const getNotesByNotebookVars: GetNotesByNotebookVariables = {
  notebookId: ..., 
};

// Call the `getNotesByNotebook()` function to execute the query.
// You can use the `await` keyword to wait for the promise to resolve.
const { data } = await getNotesByNotebook(getNotesByNotebookVars);
// Variables can be defined inline as well.
const { data } = await getNotesByNotebook({ notebookId: ..., });

// You can also pass in a `DataConnect` instance to the action shortcut function.
const dataConnect = getDataConnect(connectorConfig);
const { data } = await getNotesByNotebook(dataConnect, getNotesByNotebookVars);

console.log(data.notes);

// Or, you can use the `Promise` API.
getNotesByNotebook(getNotesByNotebookVars).then((response) => {
  const data = response.data;
  console.log(data.notes);
});
```

### Using `GetNotesByNotebook`'s `QueryRef` function

```typescript
import { getDataConnect, executeQuery } from 'firebase/data-connect';
import { connectorConfig, getNotesByNotebookRef, GetNotesByNotebookVariables } from '@dataconnect/generated';
...
// Call the `getNotesByNotebookRef()` function to get a reference to the query.
const ref = getNotesByNotebookRef(getNotesByNotebookVars);
// Variables can be defined inline as well.
const ref = getNotesByNotebookRef({ notebookId: ..., });

// You can also pass in a `DataConnect` instance to the `QueryRef` function.
const dataConnect = getDataConnect(connectorConfig);
const ref = getNotesByNotebookRef(dataConnect, getNotesByNotebookVars);
...
// Variables can be defined inline as well.
const { data } = await createNote({ notebookId: ..., title: ..., content: ..., });
...
// Variables can be defined inline as well.
const ref = createNoteRef({ notebookId: ..., title: ..., content: ..., });
...
// Variables can be defined inline as well.
const ref = updateNoteRef({ id: ..., title: ..., content: ..., });
